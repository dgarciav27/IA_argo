...
Loads splits from argo_preprocess.py output, one ocean at a time.
For each ocean:
  - Baseline XGBoost (200 trees, default threshold 0.5)
...
# SET UP
# one ocean per input, each with its own preprocessing folder and output folder
OCEANS = {
    "Atlantic": {
        "preprocess_dir": "/work/drgarcia/Dataset/atlantic_ocean/2018-2022/2.preprocessed/split_time_ascA_dmodeD",
        "model_dir":      "/work/drgarcia/Models_and_results/XG_boost/2018_2022/atlantic_f1macro_time_asc_nounder",
    },
    "Indian": {
        "preprocess_dir": "/work/drgarcia/Dataset/indian_ocean/2018-2022/2.preprocessed/split_time_ascA_dmodeD",
        "model_dir":      "/work/drgarcia/Models_and_results/XG_boost/2018_2022/indian_f1macro_time_asc_nounder",
    },
    "Pacific": {
        "preprocess_dir": "/work/drgarcia/Dataset/pacific_ocean/2018-2022/2.preprocessed/split_time_ascA_dmodeD",
        "model_dir":      "/work/drgarcia/Models_and_results/XG_boost/2018_2022/pacific_f1macro_time_asc_nounder",
    },
}
...
def train_one_ocean(ocean_name, cfg):
    ...
    #  MLFLOW SETUP
    experiment_name = f"XGBoost_{ocean_name}_2018_2022_TemporalSplit"
    ...
    #  TRAINING
    with mlflow.start_run(run_name=f"XGBoost_f1_time_ascD_nounder_Dmode_lvls10_{ocean_name.lower()}"):
        ...
        #  BASELINE
        print("\n" + "=" * 70)
        print("Baseline XGBoost (200 trees, default threshold 0.5)")
        print("=" * 70)
        ...
        # eval_set en VAL NOT IN TEST para evitar leakage en early stopping
        ...
        search = HalvingRandomSearchCV(
            ...
            cv=cv,
            scoring="f1_macro",      #  F1-macro, igual que RF
            n_jobs=1,
            verbose=2,
            random_state=42,
        )
        ...
        probs_val  = best_xgb_model.predict_proba(X_val)[:, 1]
        ...
        _save("Model",
              lambda: joblib.dump(best_xgb_model,
                                  os.path.join(output_dir, "xgb_optimized.pkl")))
        ...
        def _save_report():
            ...
            with open(path, "w") as f:
                f.write(f"XGBoost — {ocean_name} | Time split | Ascending only | Not undersampled | scoring=f1_macro \n")
                f.write("=" * 70 + "\n\n")
        ...
        _save("MLflow model log",
              lambda: mlflow.xgboost.log_model(best_xgb_model, name="xgb_optimized_model"))
